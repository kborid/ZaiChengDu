...
import com.z012.chengdu.sc.ui.activity.QAPursueErrorCorrectionActivity;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (position + 1 == mBeans.size()) {
			...
			if (mRecommand != null && !mRecommand.isEmpty()) {
				...
				int padding = DisplayUtil.dip2px(15);
				for (int i = 0; i < mRecommand.size(); i++) {
					View line = new View(mContext);
					line.setLayoutParams(new LinearLayout.LayoutParams(
							LayoutParams.MATCH_PARENT, DisplayUtil.dip2px(0.5f)));
					...
					tv.setLayoutParams(new LinearLayout.LayoutParams(
							LayoutParams.MATCH_PARENT, DisplayUtil.dip2px(49)));
					...
					tv.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							Intent intent = new Intent(mContext,
									HtmlActivity.class);
							intent.putExtra("id", mData.appid);
							intent.putExtra("path", mData.appurls);
							mContext.startActivity(intent);
						}
					});
