...
/**
 * 自定义ScrollView实现背景图片伸缩
 * 
 * @author LiaoBo
 */
public class MyScrollViewWidget extends ScrollView {
	private final String	TAG			= MyScrollViewWidget.class.getSimpleName();
	 ...
	 * @param attrs
	 */
	public MyScrollViewWidget(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	/***
	 * 根据 XML 生成视图工作实现.该函数在生成视图的最后调用，在所有子视图添加完以后. 即使子类覆盖了 onFinishInflate 方法，也应当调用父类的方法，使该方法得以执行.
	 */
	@Override
	...
	public void commOnTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_UP :
				...
				break;
			/***
			 * 消除出第一次挪动计算，因为第一次无法得知deltaY的高度， 然而我们也要进行初始化，就是第一次挪动的时候让滑动距离归0. 以后记载精确了就正常执行.
			 */
